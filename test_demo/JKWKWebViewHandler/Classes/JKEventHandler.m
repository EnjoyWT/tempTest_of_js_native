...
+ (instancetype)customInitWithHandlerDelegate:(id)delegate {
     JKEventHandler * handel =  [[self alloc]init];
    if (handel) {
        handel.handlerDelegate = delegate;
    }
    return handel;
}
- (void)cleanHandler{
    if (self.webView) {
        [self.webView evaluateJavaScript:@"JKEventHandler.removeAllCallBacks();" completionHandler:nil];//删除所有的回调事件
        [self.webView.configuration.userContentController removeScriptMessageHandlerForName:JKEventHandlerName];
    }
    self.webView = nil;
...
- (void)userContentController:(WKUserContentController *)userContentController
   ...
   #pragma clang diagnostic pop
       // NSString *plugin = message.body[@"plugin"];
        //NSString *plugin = self.hodlerClassName;
        NSString *plugin = @"";
        NSLog(@"-------6----%@",plugin);
        ...
        [self interactWithPlugin:@"" funcName:funcName params:params success:^(id response) {
...
- (void)class:(Class)class performSelector:(SEL)aSelector withObjects:(NSArray *)objects {
    ...
    [invocation setTarget:self.handlerDelegate];
